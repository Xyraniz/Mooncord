...
const layoutId = "3080264545"; // Mooncord Layout v2
...
export async function askToApplySteamLayout(win: BrowserWindow) {
    ...
    const { response } = await dialog.showMessageBox(win, {
        message: `${update ? "Update" : "Apply"} Mooncord Steam Input Layout?`,
        detail: `Would you like to ${update ? "Update" : "Apply"} Mooncord's recommended Steam Deck controller settings?
